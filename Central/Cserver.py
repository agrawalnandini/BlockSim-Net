...
class Cserver:
        ...
        if(p.Nn>1):
            for i in range(p.Nn):
                ...
                if mID==minerId:
                    ...
                    try:
                        longestBlockchain = pickle.loads(node_as_client.recv(2048))
    ...
    def getLastBlock():
        ...
        while True:
            if count<p.Nn:
                ...
                print("\nGot connection from", addr)
                blockRecv = pickle.loads(client_socket.recv(1024)) #miner id + last block
    ...
    def consensus():
        ...
        minerId=Cserver.getLastBlock()
        print("Miner ID of the miner with the longest chain: ", minerId)
        longestBlockchain=Cserver.getChain(minerId)
        ...
        while True:
            if count<p.Nn-1:
                client_socket, addr = server_socket.accept() 
                print("\nGot connection from", addr)
                client_socket.send(pickle.dumps(longestBlockchain))
                client_socket.close()   
                count+=1
            else:
                server_socket.close()
